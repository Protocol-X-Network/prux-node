...
var protocolxService = require('../../lib/services/protocolxd');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each protocolxcore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'protocolxd',
          module: protocolxService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each protocolxcore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'protocolxd',
