...
var bitcore = require('protocolxcore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var protocolxRPC = require('protocolxd-rpc');
...
/**
 * Provides a friendly event driven API to protocolxd in Node.js. Manages starting and
 * stopping protocolxd as a child process for application support, as well as connecting
 * to multiple protocolxd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function protocolx(options) {
  if (!(this instanceof protocolx)) {
    return new protocolx(options);
  }
...
util.inherits(protocolx, Service);

protocolx.dependencies = [];

protocolx.DEFAULT_MAX_TXIDS = 1000;
protocolx.DEFAULT_MAX_HISTORY = 50;
protocolx.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
protocolx.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
protocolx.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
protocolx.DEFAULT_SPAWN_RESTART_TIME = 5000;
protocolx.DEFAULT_SPAWN_STOP_TIME = 10000;
protocolx.DEFAULT_TRY_ALL_INTERVAL = 1000;
protocolx.DEFAULT_REINDEX_INTERVAL = 10000;
protocolx.DEFAULT_START_RETRY_INTERVAL = 5000;
protocolx.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
protocolx.DEFAULT_TRANSACTION_CONCURRENCY = 5;
protocolx.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
protocolx.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || protocolx.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || protocolx.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || protocolx.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || protocolx.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || protocolx.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || protocolx.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || protocolx.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || protocolx.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || protocolx.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || protocolx.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

protocolx.prototype._initCaches = function() {
...
protocolx.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
protocolx.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
protocolx.prototype.getPublishEvents = function() {
  return [
    {
...
protocolx.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'protocolxd/' + name, 'total:', this.subscriptions[name].length);
};

protocolx.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'protocolxd/' + name, 'total:', this.subscriptions[name].length);
};


protocolx.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
protocolx.prototype.unsubscribeBalance = function(emitter, addresses) {
...
protocolx.prototype.unsubscribeBalanceAll = function(emitter) {
...
protocolx.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'protocolxd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

protocolx.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'protocolxd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
protocolx.prototype.unsubscribeAddressAll = function(emitter) {
...
protocolx.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = protocolx.DEFAULT_CONFIG_SETTINGS;
...
protocolx.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
protocolx.prototype._expandRelativeDatadir = function() {
...
protocolx.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in protocolxd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in protocolxd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in protocolxd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './protocolx.conf');

  log.info('Using protocolx config file:', configPath);
...
protocolx.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of protocolxcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of protocolxcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of protocolxcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to protocolxd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from protocolxd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from protocolxd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that protocolxd is undergoing a reindex. ' +
             ...
             'of protocolxcore-node services will start.');
    node._reindex = true;
  }
};

protocolx.prototype._resetCaches = function() {
...
protocolx.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
protocolx.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

protocolx.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('protocolx Daemon Ready');
          callback();
        });
      });
...
protocolx.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 5889
  };
...
protocolx.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/protocolx.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/protocolx.conf';
    }
  }
  return networkPath;
};

protocolx.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
protocolx.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('protocolxd/hashblock', message.toString('hex'));
    }
  }

};

protocolx.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
protocolx.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('protocolx Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
protocolx.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
protocolx.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
protocolx.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
protocolx.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('protocolxd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
protocolx.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('protocolxd/rawtransaction', message.toString('hex'));
    }
...
protocolx.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || protocolx.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

protocolx.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
protocolx.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
protocolx.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('protocolx Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || protocolx.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

protocolx.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
protocolx.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/protocolxd.pid';
...
protocolx.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting protocolx process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('protocolx process unexpectedly exited with code:', code);
        log.warn('Restarting protocolx child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('protocolx process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn protocolxd.'));
      }
...
protocolx.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to protocolxd.'));
    }
 ...
 */
protocolx.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('protocolx configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
protocolx.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
protocolx.prototype.syncPercentage = function(callback) {
  var self = this;
...
protocolx.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, protocolxcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
protocolx.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
protocolx.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
protocolx.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
protocolx.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in protocolxcore is most recent to less recent
 ...
 * @param {Function} callback
 */
protocolx.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
protocolx.prototype._getConfirmationsDetail = function(transaction) {
...
protocolx.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
protocolx.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
protocolx.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A protocolxcoin transaction id
 * @param {Function} callback
 */
protocolx.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
protocolx.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
protocolx.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
protocolx.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
protocolx.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
protocolx.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
protocolx.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
protocolx.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
protocolx.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
protocolx.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a protocolxcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
protocolx.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getInfo = function(callback) {
...
protocolx.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('protocolxd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('protocolxd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
protocolx.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
protocolx.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
protocolx.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
protocolx.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
protocolx.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
protocolx.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
protocolx.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
protocolx.prototype.getStakingInfo = function(callback) {
    var self = this,
