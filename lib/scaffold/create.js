...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'protocolxcore-lib': 'https://github.com/protocol-x-network/protocolxcore-lib.git#master',
    'protocolxcore-node': 'https://github.com/protocol-x-network/protocolxcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/protocolxcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the protocolxcore-node directory and configuration
