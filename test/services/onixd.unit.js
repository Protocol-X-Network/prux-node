...
var protocolxcore = require('protocolxcore-lib');
var _ = protocolxcore.deps._;
...
var Transaction = protocolxcore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/protocolx.conf')));
var protocolxService = proxyquire('../../lib/services/protocolxd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultprotocolxConf = fs.readFileSync(path.resolve(__dirname, '../data/default.protocolx.conf'), 'utf8');

describe('protocolx Service', function() {
	...
	var baseConfig = {
		node: {
			network: protocolxcore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var protocolxd = new protocolxService(baseConfig);
			should.exist(protocolxd);
		});
		it('will create an instance without `new`', function() {
			var protocolxd = protocolxService(baseConfig);
			should.exist(protocolxd);
		});
		it('will init caches', function() {
			var protocolxd = new protocolxService(baseConfig);

			should.exist(protocolxd.utxosCache);
			should.exist(protocolxd.txidsCache);
			should.exist(protocolxd.balanceCache);
			should.exist(protocolxd.summaryCache);
			should.exist(protocolxd.transactionDetailedCache);
			should.exist(protocolxd.accountInfo);

			should.exist(protocolxd.transactionCache);
			should.exist(protocolxd.rawTransactionCache);
			should.exist(protocolxd.rawJsonTransactionCache);
			should.exist(protocolxd.blockCache);
			should.exist(protocolxd.blockJsonCache);
			should.exist(protocolxd.blockSubsidyCache);
			should.exist(protocolxd.rawBlockCache);
			should.exist(protocolxd.blockHeaderCache);
			should.exist(protocolxd.zmqKnownTransactions);
			should.exist(protocolxd.zmqKnownBlocks);
			should.exist(protocolxd.lastTip);
			should.exist(protocolxd.lastTipTimeout);
			should.equal(protocolxd.dgpInfoCache, null, 'should be null');
			should.equal(protocolxd.miningInfoCache, null, 'should be null');
			should.equal(protocolxd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(protocolxd.maxTxids, 1000);
			should.equal(protocolxd.maxTransactionHistory, 50);
			should.equal(protocolxd.maxAddressesQuery, 10000);
			should.equal(protocolxd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(protocolxd.spawnRestartTime, 5000);
			should.equal(protocolxd.spawnStopTime, 10000);

			// try all interval
			should.equal(protocolxd.tryAllInterval, 1000);
			should.equal(protocolxd.startRetryInterval, 5000);

			// rpc limits
			should.equal(protocolxd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(protocolxd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.nodes.should.deep.equal([]);
			protocolxd.nodesIndex.should.equal(0);
			protocolxd.nodes.push({ client: sinon.stub() });
			should.exist(protocolxd.client);
		});
		it('will set subscriptions', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._initDefaults({ transactionConcurrency: 10 });
			protocolxd.transactionConcurrency.should.equal(10);
			protocolxd._initDefaults({});
			protocolxd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			protocolxService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var protocolxd = new protocolxService(baseConfig);
			var methods = protocolxd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var protocolxd = new protocolxService(baseConfig);
			var events = protocolxd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('protocolxd/rawtransaction');
			events[0].scope.should.equal(protocolxd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('protocolxd/hashblock');
			events[1].scope.should.equal(protocolxd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('protocolxd/addresstxid');
			events[2].scope.should.equal(protocolxd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('protocolxd/addressbalance');
			events[3].scope.should.equal(protocolxd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.subscribe = sinon.stub();
			protocolxd.unsubscribe = sinon.stub();
			var events = protocolxd.getPublishEvents();

			events[0].subscribe('test');
			protocolxd.subscribe.args[0][0].should.equal('rawtransaction');
			protocolxd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			protocolxd.unsubscribe.args[0][0].should.equal('rawtransaction');
			protocolxd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			protocolxd.subscribe.args[1][0].should.equal('hashblock');
			protocolxd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			protocolxd.unsubscribe.args[1][0].should.equal('hashblock');
			protocolxd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var protocolxd = new protocolxService(baseConfig);
			var emitter = {};
			protocolxd.subscribe('hashblock', emitter);
			protocolxd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			protocolxd.subscribe('rawtransaction', emitter2);
			protocolxd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			protocolxd.subscribe('hashblock', emitter1);
			protocolxd.subscribe('hashblock', emitter2);
			protocolxd.subscribe('hashblock', emitter3);
			protocolxd.subscribe('hashblock', emitter4);
			protocolxd.subscribe('hashblock', emitter5);
			protocolxd.subscriptions.hashblock.length.should.equal(5);

			protocolxd.unsubscribe('hashblock', emitter3);
			protocolxd.subscriptions.hashblock.length.should.equal(4);
			protocolxd.subscriptions.hashblock[0].should.equal(emitter1);
			protocolxd.subscriptions.hashblock[1].should.equal(emitter2);
			protocolxd.subscriptions.hashblock[2].should.equal(emitter4);
			protocolxd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var protocolxd = new protocolxService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			protocolxd.subscriptions.hashblock = [emitter1];
			protocolxd.unsubscribe('hashblock', emitter3);
			protocolxd.subscriptions.hashblock.length.should.equal(1);
			protocolxd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			protocolxd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(protocolxd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var protocolxd = new protocolxService(baseConfig);
			var emitter = new EventEmitter();
			protocolxd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var protocolxd = new protocolxService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			protocolxd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			protocolxd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			protocolxd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.unsubscribeAddress(emitter1);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			protocolxd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(protocolxd, 'unsubscribeAddressAll');
			protocolxd.unsubscribeAddress(emitter1);
			protocolxd.unsubscribeAddressAll.callCount.should.equal(1);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			protocolxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			protocolxd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			protocolxd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			protocolxd.unsubscribeAddressAll(emitter1);
			protocolxd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			protocolxd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			protocolxd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(protocolxd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var protocolxd = new protocolxService(baseConfig);
			var config = protocolxd._getDefaultConfig();
			config.should.equal(defaultprotocolxConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a protocolx.conf file', function() {
			var Testprotocolx = proxyquire('../../lib/services/protocolxd', {
				fs: {
					readFileSync: readFileSync,
			...
			var protocolxd = new Testprotocolx(baseConfig);
			protocolxd.options.spawn.datadir = '/tmp/.protocolx';

			sinon.spy(protocolxd, '_expandRelativeDatadir');
			sinon.spy(protocolxd, '_checkConfigIndexes');
			sinon.spy(protocolxd, '_getDefaultConf')
			sinon.spy(protocolxd, '_getNetworkConfigPath');
			sinon.spy(protocolxd, '_parseBitcoinConf');

			var node = {};
			protocolxd._loadSpawnConfiguration(node);

			protocolxd._expandRelativeDatadir.callCount.should.equal(1);
			protocolxd._checkConfigIndexes.callCount.should.equal(1);
			protocolxd._getDefaultConf.callCount.should.equal(1);
			protocolxd._getNetworkConfigPath.callCount.should.equal(1);
			protocolxd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(protocolxd.spawn.config);
			protocolxd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: protocolxcore.Networks.testnet,
					configPath: '/tmp/.protocolxcore/protocolxcore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var protocolxd = new Testprotocolx(config);
			...
			protocolxd.options.spawn.datadir = './data';
			var node = {};
			protocolxd._loadSpawnConfiguration(node);
			...
			protocolxd.options.spawn.datadir.should.equal('/tmp/.protocolxcore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var Testprotocolx = proxyquire('../../lib/services/protocolxd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badprotocolx.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var protocolxd = new Testprotocolx(baseConfig);
			(function() {
				protocolxd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(protocolxcore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the protocolx.conf file', function() {
			var protocolxd = new protocolxService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				protocolxd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the protocolx.conf file', function() {
			var protocolxd = new protocolxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the protocolx.conf file', function() {
			var protocolxd = new protocolxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the protocolx.conf file', function() {
			var protocolxd = new protocolxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the protocolx.conf file', function() {
			var protocolxd = new protocolxService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				protocolxd.transactionDetailedCache.set(keys[i], {});
				protocolxd.utxosCache.set(keys[i], {});
				protocolxd.accountInfo.set(keys[i], {});
				protocolxd.txidsCache.set(keys[i], {});
				protocolxd.balanceCache.set(keys[i], {});
				protocolxd.summaryCache.set(keys[i], {});
				protocolxd.blockOverviewCache.set(keys[i], {});
				protocolxd.blockJsonCache.set(keys[i], {});
			}
			protocolxd._resetCaches();
			should.equal(protocolxd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(protocolxd.utxosCache.get(keys[0]), undefined);
			should.equal(protocolxd.accountInfo.get(keys[0]), undefined);
			should.equal(protocolxd.txidsCache.get(keys[0]), undefined);
			should.equal(protocolxd.balanceCache.get(keys[0]), undefined);
			should.equal(protocolxd.summaryCache.get(keys[0]), undefined);
			should.equal(protocolxd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(protocolxd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.tryAllInterval = 1;
			protocolxd.nodes.push({
				client: {
			...
			protocolxd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				protocolxd.nodes[0].client.getInfo.callCount.should.equal(1);
				protocolxd.nodes[1].client.getInfo.callCount.should.equal(1);
				protocolxd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			protocolxd._tryAllClients(function(client, next) {
				...
				protocolxd.nodes[1].client.getInfo.callCount.should.equal(1);
				protocolxd.nodes[2].client.getInfo.callCount.should.equal(1);
				protocolxd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert protocolxd-rpc error object into JavaScript error', function() {
			var protocolxd = new protocolxService(baseConfig);
			var error = protocolxd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			protocolxd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			protocolxd.nodes.push({
				client: {
			...
			protocolxd._initChain(function() {
				log.info.callCount.should.equal(1);
				protocolxd.getRawBlock.callCount.should.equal(1);
				protocolxd.getRawBlock.args[0][0].should.equal('genesishash');
				protocolxd.height.should.equal(5000);
				protocolxd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			protocolxd._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			protocolxd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			protocolxd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			protocolxcore.Networks.disableRegtest();
			baseConfig.node.network = protocolxcore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: protocolxcore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var protocolxd = new protocolxService(config);
			protocolxd._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var protocolxd = new protocolxService(config);
			protocolxd._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			protocolxcore.Networks.enableRegtest();
			var config = {
				node: {
					network: protocolxcore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var protocolxd = new protocolxService(config);
			should.equal(protocolxd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var protocolxd = new protocolxService(config);
			protocolxd._getNetworkConfigPath().should.equal('testnet3/protocolx.conf');
		});
		it('will get default rpc port for regtest', function() {
			protocolxcore.Networks.enableRegtest();
			var config = {
			...
			var protocolxd = new protocolxService(config);
			protocolxd._getNetworkConfigPath().should.equal('regtest/protocolx.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.node.network = protocolxcore.Networks.testnet;
			protocolxd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.node.network = protocolxcore.Networks.testnet;
			protocolxcore.Networks.enableRegtest();
			protocolxd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.node.network = protocolxcore.Networks.livenet;
			protocolxcore.Networks.enableRegtest();
			should.equal(protocolxd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			protocolxd._rapidProtectedUpdateTip = sinon.stub();
			protocolxd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			protocolxd._zmqBlockHandler(node, message);
			protocolxd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			protocolxd._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(protocolxd, 'emit');
			protocolxd.on('block', function(block) {
				block.should.equal(message);
				protocolxd.emit.callCount.should.equal(1);
				protocolxd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			protocolxd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			protocolxd._zmqBlockHandler(node, message);
			protocolxd._rapidProtectedUpdateTip.callCount.should.equal(1);
			protocolxd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			protocolxd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(protocolxd, 'emit');
			var emitter = new EventEmitter();
			protocolxd.subscriptions.hashblock.push(emitter);
			emitter.on('protocolxd/hashblock', function(blockHash) {
				protocolxd._rapidProtectedUpdateTip.callCount.should.equal(1);
				protocolxd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			var callCount = 0;
			protocolxd._updateTip = function() {
				callCount++;
			...
			var count = 0;
			function repeat() {
				protocolxd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub();
			sinon.spy(protocolxd, '_resetCaches');
			protocolxd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				protocolxd._resetCaches.callCount.should.equal(1);
			...
			protocolxd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			protocolxd.on('synced', function() {
				protocolxd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			protocolxd.on('synced', function() {
				protocolxd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			protocolxd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				protocolxd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub();
			protocolxd._resetCaches = sinon.stub();
			protocolxd.on('tip', function(height) {
				protocolxd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				protocolxd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using protocolxcore.Transaction', function() {
			var protocolxd = new protocolxService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = protocolxcore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(protocolxcore.Networks.testnet);
			var outputAddress = protocolxcore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = protocolxcore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: protocolxcore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = protocolxd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var protocolxd = new protocolxService(baseConfig);
			var tx = protocolxcore.Transaction();
			tx.addInput(protocolxcore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: protocolxcore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: protocolxcore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(protocolxcore.Transaction.Output({
				script: protocolxcore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = protocolxd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var protocolxd = new protocolxService(baseConfig);
			var tx = protocolxcore.Transaction();
			var address = protocolxcore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(protocolxcore.Transaction.Output({
				script: protocolxcore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(protocolxcore.Transaction.Output({
				script: protocolxcore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			protocolxd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			protocolxd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('protocolxd/addresstxid', function(data) {
			...
			protocolxd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			protocolxd.subscriptions.rawtransaction.push(emitter);
			emitter.on('protocolxd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			protocolxd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(protocolxd, 'emit');
			protocolxd.subscriptions.rawtransaction.push(emitter);
			emitter.on('protocolxd/rawtransaction', function() {
				done();
			});
			var node = {};
			protocolxd._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			protocolxd.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._updateTip = sinon.stub();
			protocolxd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			protocolxd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			protocolxd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				protocolxd._updateTip.callCount.should.equal(11);
				protocolxd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			protocolxd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				protocolxd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			protocolxd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				protocolxd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			protocolxd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				protocolxd._zmqBlockHandler.callCount.should.equal(0);
				protocolxd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var protocolxd = new protocolxService(baseConfig);
			var node = {};
			protocolxd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			protocolxd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			protocolxd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			protocolxd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			protocolxd.on('tip', function(height) {
				...
				protocolxd.height.should.equal(100);
				done();
			});
			protocolxd._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestprotocolxService = proxyquire('../../lib/services/protocolxd', {
				fs: {
					readFile: readFile
				}
			});
			var protocolxd = new TestprotocolxService(baseConfig);
			protocolxd.spawnStopTime = 1;
			protocolxd._process = {};
			protocolxd._process.kill = sinon.stub();
			protocolxd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				protocolxd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			protocolxd._process.kill = sinon.stub().throws(error2);
			protocolxd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._loadSpawnConfiguration = sinon.stub();
			protocolxd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			protocolxd._spawnChildProcess(function(err) {
				protocolxd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._loadSpawnConfiguration = sinon.stub();
			protocolxd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			protocolxd._spawnChildProcess(function(err) {
				protocolxd._loadSpawnConfiguration.callCount.should.equal(1);
				protocolxd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var protocolxd = new TestprotocolxService(config);
			protocolxd.spawn = {};
			protocolxd._loadSpawnConfiguration = sinon.stub();
			protocolxd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			protocolxd.node.stopping = true;
			protocolxd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			protocolxd._loadSpawnConfiguration = sinon.stub();
			protocolxd.spawn = {};
			protocolxd.spawn.exec = 'testexec';
			protocolxd.spawn.configPath = 'testdir/protocolx.conf';
			protocolxd.spawn.datadir = 'testdir';
			protocolxd.spawn.config = {};
			protocolxd.spawn.config.rpcport = 20001;
			protocolxd.spawn.config.rpcuser = 'protocolx';
			protocolxd.spawn.config.rpcpassword = 'password';
			protocolxd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			protocolxd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			protocolxd._initZmqSubSocket = sinon.stub();
			protocolxd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			protocolxd._checkReindex = sinon.stub().callsArgWith(1, null);
			protocolxd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/protocolx.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				protocolxd._loadTipFromNode.callCount.should.equal(1);
				protocolxd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(protocolxd._initZmqSubSocket.args[0][0].client);
				protocolxd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				protocolxd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(protocolxd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn protocolxd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestprotocolxService = proxyquire('../../lib/services/protocolxd', {
				fs: {
			...
			protocolxd._loadSpawnConfiguration = sinon.stub();
			protocolxd.spawn = {};
			protocolxd.spawn.exec = 'protocolxd';
			protocolxd.spawn.datadir = '/tmp/protocolx';
			protocolxd.spawn.configPath = '/tmp/protocolx/protocolx.conf';
			protocolxd.spawn.config = {};
			protocolxd.spawnRestartTime = 1;
			protocolxd._loadTipFromNode = sinon.stub().callsArg(1);
			protocolxd._initZmqSubSocket = sinon.stub();
			protocolxd._checkReindex = sinon.stub().callsArg(1);
			protocolxd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			protocolxd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(protocolxd, '_spawnChildProcess');
			protocolxd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						protocolxd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			protocolxd._spawnChildProcess(function(err) {
				...
				protocolxd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				protocolxd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn protocolxd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestprotocolxService = proxyquire('../../lib/services/protocolxd', {
				fs: {
			...
			protocolxd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						protocolxd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			var protocolxd = new TestprotocolxService(baseConfig);
			protocolxd.startRetryInterval = 1;
			protocolxd._loadSpawnConfiguration = sinon.stub();
			protocolxd.spawn = {};
			protocolxd.spawn.exec = 'testexec';
			protocolxd.spawn.configPath = 'testdir/protocolx.conf';
			...
			protocolxd.spawn.config.rpcpassword = 'password';
			protocolxd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			protocolxd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			protocolxd._spawnChildProcess(function(err) {
				protocolxd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			protocolxd._initZmqSubSocket = sinon.stub();
			protocolxd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			protocolxd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var protocolxd = new protocolxService(config);
			protocolxd.node.stopping = true;
			protocolxd.startRetryInterval = 100;
			protocolxd._loadTipFromNode = sinon.stub();
			protocolxd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to protocolxd/);
				protocolxd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			protocolxd.startRetryInterval = 1;
			var config = {};
			protocolxd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				protocolxd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			protocolxd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			protocolxd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				protocolxd._loadTipFromNode.callCount.should.equal(1);
				protocolxd._initZmqSubSocket.callCount.should.equal(1);
				protocolxd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.options = {};
			protocolxd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('protocolx configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			protocolxd.options = {
				connect: [
					{}
				]
			...
			protocolxd.start(function(err) {
				protocolxd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			var node = {};
			protocolxd._initChain = sinon.stub().callsArg(0);
			protocolxd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			protocolxd.options = {
				spawn: {}
			};
			protocolxd.start(function(err) {
				should.not.exist(err);
				protocolxd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(protocolxd, '_spawnChildProcess');
			var nodes = [{}];
			protocolxd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			protocolxd.options = {
				connect: [
					{}
				]
			...
			protocolxd.start(function(err) {
				should.not.exist(err);
				protocolxd._initChain.callCount.should.equal(1);
				protocolxd._connectProcess.callCount.should.equal(1);
				protocolxd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			protocolxd.isSynced(function(err) {
				should.exist(err);
				protocolxd.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			protocolxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				protocolxd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			protocolxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			protocolxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			protocolxd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.syncPercentage(function(err) {
				should.exist(err);
				protocolxd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			protocolxd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				protocolxd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var protocolxd = new protocolxService(baseConfig);
			var args = protocolxd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var protocolxd = new protocolxService(baseConfig);
			var args = protocolxd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_normalizeAddressArg');
			protocolxd.nodes.push({
				client: {
			...
			var options = {};
			protocolxd.getAddressBalance(address, options, function(err) {
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			protocolxd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				protocolxd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					protocolxd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			protocolxd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = protocolxd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = protocolxd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = protocolxd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = protocolxd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				protocolxd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			protocolxd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(protocolxd, '_getHeightRangeQuery');
			sinon.spy(protocolxd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			protocolxd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				protocolxd._getHeightRangeQuery.callCount.should.equal(1);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			protocolxd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				protocolxd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			protocolxd.getAddressTxids(address, options, function(err, txids) {
				...
				protocolxd.getAddressTxids(address, options, function(err, txids) {
					...
					protocolxd._getHeightRangeQuery.callCount.should.equal(2);
					protocolxd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			protocolxd.getAddressTxids(address, options, function(err, txids) {
				...
				protocolxd.getAddressTxids(address, options, function(err, txids) {
					...
					protocolxd._getHeightRangeQuery.callCount.should.equal(4);
					protocolxd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			protocolxd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				protocolxd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					protocolxd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						protocolxd._getHeightRangeQuery.callCount.should.equal(3);
						protocolxd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			tx.height = -1;
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.height = 10;
			var confirmations = protocolxd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var protocolxd = new protocolxService(baseConfig);
			var result = {};
			protocolxd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var protocolxd = new protocolxService(baseConfig);
			var result = {};
			protocolxd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var protocolxd = new protocolxService(baseConfig);
			var result = {};
			protocolxd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var protocolxd = new protocolxService(baseConfig);
			var result = {};
			protocolxd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(protocolxd, '_getAddressDetailsForInput');
			sinon.spy(protocolxd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = protocolxd._getAddressDetailsForTransaction(tx, addresses);
			...
			protocolxd._getAddressDetailsForInput.callCount.should.equal(1);
			protocolxd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_getConfirmationsDetail');
			protocolxd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			protocolxd.height = 300;
			var addresses = {};
			protocolxd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			protocolxd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				protocolxd.getDetailedTransaction.callCount.should.equal(1);
				protocolxd._getAddressDetailsForTransaction.callCount.should.equal(1);
				protocolxd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			protocolxd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				protocolxd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from protocolxcore addresses', function() {
			var addresses = [
				protocolxcore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				protocolxcore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var protocolxd = new protocolxService(baseConfig);
			var strings = protocolxd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var protocolxd = new protocolxService(baseConfig);
			(function() {
				protocolxd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_normalizeAddressArg');
			protocolxd.maxAddressesQuery = 100;
			protocolxd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_normalizeAddressArg');
			sinon.spy(protocolxd, '_getAddressStrings');
			protocolxd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(protocolxd, '_normalizeAddressArg');
			sinon.spy(protocolxd, '_getAddressStrings');
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			protocolxd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(protocolxd, '_normalizeAddressArg');
			sinon.spy(protocolxd, '_getAddressStrings');
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			protocolxd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			protocolxd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd._getAddressStrings.callCount.should.equal(1);
				protocolxd.getAddressTxids.callCount.should.equal(1);
				protocolxd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(protocolxd, '_normalizeAddressArg');
			sinon.spy(protocolxd, '_getAddressStrings');
			sinon.spy(protocolxd, 'getAddressTxids');
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			protocolxd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(protocolxd, '_normalizeAddressArg');
			sinon.spy(protocolxd, '_getAddressStrings');
			sinon.spy(protocolxd, '_paginateTxids');
			protocolxd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(protocolxd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			protocolxd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd._getAddressStrings.callCount.should.equal(1);
				protocolxd.getAddressTxids.callCount.should.equal(1);
				protocolxd._getAddressDetailedTransaction.callCount.should.equal(2);
				protocolxd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			protocolxd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			protocolxd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			protocolxd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			protocolxd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			protocolxd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			protocolxd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			protocolxd.getAddressSummary(address, options, function(err, summary) {
				protocolxd._paginateTxids.callCount.should.equal(1);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd.getAddressTxids.callCount.should.equal(1);
				protocolxd.getAddressBalance.callCount.should.equal(1);
				protocolxd._paginateTxids.args[0][1].should.equal(0);
				protocolxd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			protocolxd.getAddressSummary(address, options, function(err, summary) {
				...
				protocolxd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					protocolxd._paginateTxids.callCount.should.equal(0);
					protocolxd._normalizeAddressArg.callCount.should.equal(2);
					protocolxd.getAddressTxids.callCount.should.equal(1);
					protocolxd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			protocolxd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				protocolxd._paginateTxids.callCount.should.equal(1);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				protocolxd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			protocolxd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			protocolxd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(protocolxd, '_tryAllClients');
			protocolxd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			protocolxd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			protocolxd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			protocolxd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			protocolxd.getBlock(0, function(err) {
				protocolxd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as protocolxcore object from height', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			protocolxd.getBlock(0, function(err, block) {
				should.not.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(protocolxcore.Block);
				done();
			});
		});
		it('will getblock as protocolxcore object', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			protocolxd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			protocolxd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			protocolxd.getBlock(0, function(err, block) {
				...
				protocolxd.getBlock(0, function(err, block) {
					should.not.exist(err);
					protocolxd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			protocolxd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			protocolxd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			protocolxd.getBlockHeader(10, function(err) {
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			protocolxd.getBlockHeader(blockhash, function(err) {
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			protocolxd.getBlockHeader(0, function(err) {
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			protocolxd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			protocolxd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			protocolxd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				protocolxd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			protocolxd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			protocolxd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_maybeGetBlockHash');
			protocolxd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			protocolxd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			protocolxd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				protocolxd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					protocolxd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			protocolxd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = protocolxcore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			protocolxd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			protocolxd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			protocolxd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = protocolxcore.Transaction();
			(function() {
				protocolxd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getRawTransaction('txid', function(err) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			protocolxd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getTransaction('txid', function(err) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			protocolxd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				protocolxd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(protocolxcore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			protocolxd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				protocolxd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			protocolxd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			protocolxd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			protocolxd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getInfo and give result', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			protocolxd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			protocolxd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			protocolxd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				protocolxd.getSubsidy.callCount.should.equal(1);
				protocolxd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			protocolxd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		it('will callback if spawn is not set', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.stop(done);
		});
		it('will exit spawned process', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.spawn = {};
			protocolxd.spawn.process = new EventEmitter();
			protocolxd.spawn.process.kill = sinon.stub();
			protocolxd.stop(done);
			protocolxd.spawn.process.kill.callCount.should.equal(1);
			protocolxd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			protocolxd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			protocolxd.spawn.process.kill.callCount.should.equal(1);
			protocolxd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			protocolxd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var protocolxd = new protocolxService(baseConfig);
			protocolxd.shutdownTimeout = 300;
			protocolxd.spawn = {};
			protocolxd.spawn.process = new EventEmitter();
			protocolxd.spawn.process.kill = sinon.stub();
			protocolxd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_normalizeAddressArg');
			sinon.spy(protocolxd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			protocolxd.nodes.push({
			...
			protocolxd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				protocolxd._getBalanceFromMempool.callCount.should.equal(1);
				protocolxd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			protocolxd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			protocolxd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			protocolxd.getJsonBlock(0, function(err) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			protocolxd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			protocolxd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(2);
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			protocolxd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
				protocolxd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var protocolxd = new protocolxService(baseConfig);
			var emitter = new EventEmitter();
			protocolxd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(protocolxd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var protocolxd = new protocolxService(baseConfig);
			var emitter = new EventEmitter();
			protocolxd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			protocolxd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			protocolxd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			protocolxd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.unsubscribeBalance(emitter1);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			protocolxd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var protocolxd = new protocolxService(baseConfig);
			...
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			protocolxd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var protocolxd = new protocolxService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(protocolxd, 'unsubscribeBalanceAll');
			protocolxd.unsubscribeBalance(emitter1);
			protocolxd.unsubscribeBalanceAll.callCount.should.equal(1);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			protocolxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			protocolxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			protocolxd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			protocolxd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			protocolxd.unsubscribeBalanceAll(emitter1);
			protocolxd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			protocolxd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			protocolxd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(protocolxd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var protocolxd = new protocolxService(baseConfig);
			sinon.spy(protocolxd, '_notifyBalanceSubscriber');
			protocolxd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			protocolxd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				protocolxd._notifyBalanceSubscriber.callCount.should.equal(1);
				protocolxd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			protocolxd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				protocolxd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			protocolxd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			protocolxd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getDgpInfo(function(err, result) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getMiningInfo(function(err, result) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			protocolxd.getStakingInfo(function(err, result) {
				should.exist(err);
				protocolxd._tryAllClients.callCount.should.equal(1);
