...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for protocolxd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and protocolxcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed protocolxcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './protocolxcore-node.json');
  ...
  node.on('ready', function() {
    log.info('protocolxcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in protocolxcore-node services directory
    ...
    if (servicePackage.protocolxcoreNode) {
      serviceModule = service.name + '/' + servicePackage.protocolxcoreNode;
    }
